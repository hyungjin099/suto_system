/* 관리자 - 원단 별칭 관리
 * 드래그 앤 드롭으로 공식 원단명과 고객사별 호칭을 연결한다.
 * 상태: aliases { [clientId]: { [productId]: aliasName } }
 */
...
export default function FabricAliasAdmin() {
  ...
  const [pendingDrop, setPendingDrop] = useState(null); // product | null
  ...
  const confirmPending = () => {
    ...
    if (!alias) {
      setPendingError("별칭을 입력해 주세요.");
      return;
    }
    if (Object.values(clientAliases).includes(alias)) {
      setPendingError("이미 사용 중인 별칭입니다.");
      return;
    }
    setAliases((prev) => ({
      ...prev,
  ...
  const confirmEdit = () => {
    ...
    if (!alias) {
      setEditingError("별칭을 입력해 주세요.");
      return;
    }
    const others = Object.entries(clientAliases)
      .filter(([k]) => k !== editingId)
      .map(([, v]) => v);
    if (others.includes(alias)) {
      setEditingError("이미 사용 중인 별칭입니다.");
      return;
    }
    setAliases((prev) => ({
      ...prev,
        ...
        {clients.map((c) => {
          ...
          return (
            <button
              ...
              className={cx(
                styles.clientTab,
                c.id === selectedClientId && styles.clientTabActive
              )}
            >
              {c.name}
              {cnt > 0 && (
              ...
              <div className={styles.aliasCardPending}>
                ...
                <div className={styles.aliasInputRow}>
                  ...
                  <input
                    ...
                    className={cx(
                      styles.aliasInput,
                      pendingError && styles.aliasInputError
                    )}
                    value={pendingAlias}
                    onChange={(e) => {
                  ...
                  <button
                    type="button"
                    className={styles.cancelAliasBtn}
                ...
                {pendingError && (
                  <div className={styles.aliasError}>{pendingError}</div>
                )}
              </div>
            )}
            ...
            {filteredRight.length === 0 && !pendingDrop && (
              mappedFabrics.length === 0 ? (
                <div className={cx(styles.emptyRight, dragOver && styles.emptyRightActive)}>
                  ...
                  <div className={styles.emptyRightSub}>
                    놓으면 별칭 이름을 입력할 수 있습니다
                  </div>
                </div>
              ) : (
                ...
                {editingId === fabric.id ? (
                  <>
                    <div className={styles.aliasInputRow}>
                      ...
                      <input
                        ...
                        className={cx(
                          styles.aliasInput,
                          editingError && styles.aliasInputError
                        )}
                        value={editingValue}
                        onChange={(e) => {
                    ...
                    {editingError && (
                      <div className={styles.aliasError}>{editingError}</div>
                    )}
                  </>
                ) : (
                  <div className={styles.aliasViewRow}>
                    ...
                    <span className={styles.aliasValue}>
                      {clientAliases[fabric.id]}
                    </span>
                    <div className={styles.aliasActions}>
                      <button
                        type="button"
                        className={styles.editAliasBtn}
                        onClick={() => startEdit(fabric.id)}
                        title="별칭 수정"
                      >
                        <IconEdit size={13} />
                      </button>
